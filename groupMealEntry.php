...
class groupMealEntry extends oodbconfig{
    private $oodbconfig;
    private $today;
    private $managerId;
    private $groupId;
    ...
    public function enterTodayFoodTime($selected_time,$noOfMeal){
        ...
        $insertSelectedTimeQuery->bind_param('sssiii',$answer,$selected_time,$this->today,$noOfMeal,$this->managerId,$this->groupId);
    ...
    public function removeTodayFoodTime($selected_removal_time){
        ...
        $deleteSelectedTimeQuery="DELETE FROM mealstatus WHERE mealtime='$selected_removal_time' and mealentrytime=CURRENT_DATE and userGroupId='$this->groupId' and user_id='$this->managerId'";
    ...
    public function checkMealTimeDate($date, $time, $noOfMeal)
    {
        ...
        AND userGroupId='$this->groupId' and user_id='$this->managerId'";
    ...
    public function saveSpecificDateMealStatus($date, $selected_time, $noOfMeal,$answer)
    {
        ...
        $sql = $conn->prepare(
            "insert into mealstatus(answer,mealtime,mealentrytime, noOfMeal,user_id,userGroupid) VALUES (?,?,?,?,?,?)");
        $sql->bind_param('sssiii', $answer, $selected_time, $date, $noOfMeal, $this->managerId, $this->groupId);
        $result = $sql->execute();
        if ($result) {
    ...
    public function updateUserMealEntry($tableId,$selected_date,$noOfMeal){
        ...
        $updateQuery->bind_param('siiii',$selected_date,$noOfMeal,$this->managerId,$this->groupId,$tableId);
